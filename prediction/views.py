...
def download_report_pdf(request, report_id):
    ...
    Includes: patient info, top 3 predictions, symptoms,
              full prescription (medicine + dosage + frequency).
    ...
    title_style = ParagraphStyle('Title', parent=styles['Title'],
        fontSize=22, textColor=colors.HexColor('#2563eb'), spaceAfter=4)
    h2_style = ParagraphStyle('H2', parent=styles['Heading2'],
        fontSize=14, textColor=colors.HexColor('#1e293b'), spaceBefore=16, spaceAfter=8)
    body_style = ParagraphStyle('Body', parent=styles['Normal'],
        fontSize=11, textColor=colors.HexColor('#475569'), leading=16)
    ...
    patient_data = [
        ['Patient Name', user.name or 'N/A'],
        ['Email',        user.email],
        ['Age',          str(user.age or 'N/A')],
        ['Gender',       user.gender.title() if user.gender else 'N/A'],
        ['Contact',      user.contact_no or 'N/A'],
        ['Report Date',  str(report.report_date)],
        ['Report ID',    f'RPT-{report.id:04d}'],
    ]
    pt = Table(patient_data, colWidths=[150, 370])
    ...
    pred_data = [['Rank', 'Predicted Disease', 'Confidence Score']]
    for rank, (disease, conf) in enumerate([
        ...
        if disease:
            pred_data.append([f'#{rank}', disease.replace('_', ' ').title(), f'{conf}%'])

    prt = Table(pred_data, colWidths=[60, 300, 160])
    ...
    if prescriptions.exists():
        ...
        for rx in prescriptions:
            elements.append(Paragraph(f"<b>Primary Diagnosis:</b> {pred.predicted_disease_1.replace('_',' ').title()}", body_style))
            elements.append(Spacer(1, 8))

            rx_data = [
                ['Medicine(s)',  rx.medicine_name],
                ['Dosage',       rx.dosage],
                ['Frequency',    rx.frequency],
                ['Prescribed',   str(rx.prescribed_date.date())],
            ]
            rxt = Table(rx_data, colWidths=[120, 400])
    ...
    # ── Disclaimer ────────────────────────────────────────────
    elements.append(HRFlowable(width="100%", thickness=1, color=colors.HexColor('#e2e8f0'), spaceBefore=10))
    elements.append(Spacer(1, 8))
    elements.append(Paragraph(
        "⚠️ DISCLAIMER: This report is AI-generated by MediSense AI and is for informational purposes only. "
